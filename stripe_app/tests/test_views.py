...
class TestViews(TestCase):
    def setUp(self):
        organization = Organization.objects.create(
            org_id="PAM-123", name="Pass the Org"
        )
        Payment.objects.create(
            organization=organization, charge_amount=100, charge_type="ONBOARDING"
        )
    ...
    def test_client_secret_view(self, mock_create_payment_intent):
        ...
        response = self.client.post(
            "/stripe/payment/secret/", data=data, content_type="application/json"
        )
    ...
    @patch("stripe_app.views.payment.create_payment_intent")
    def test_client_secret_view_raises_payment_intent_error(
        ...
        mock_create_payment_intent.side_effect = PaymentIntentError(
    ...
    def test_organization_onboarding_charge_for_duplicate_charge(self):
        Payment.objects.filter(
            organization__org_id="PAM-123", charge_type="ONBOARDING"
        ).update(paid=True)
        response = self.client.get("/stripe/organization/PAM-123/charge/")
        self.assertEquals(
    ...
    def test_payment_status(self, mock_get_payment_intent):
        stripe_payment_intent_resp = {
            "id": "1239-SJDKD",
            "metadata": {"organization_id": "PAM-123", "charge_type": "ONBOARDING"},
            "charges": {
                "data": [
                    {
                        "paid": True,
